...
$Filename= FileOpenDialog("Select files to hash", "", "All Files (*.*)", 5)
	...
	if $multiple_files > 0 Then
		...
		$hashes=""
		While $j < $files[0]+1
			$hash=CRC32($folder & "\" &$files[$j])
			$lines &= $files[$j] & " = " & $hash & @CRLF
			$hashes &= $files[$j] & " = " & $hash & @CRLF
			$j=$j+1
		Wend
		$lines &= "------------------------------- End "& @MDAY & "-" & @MON & "-" & @YEAR & " (" & @HOUR & "h" & @MIN & "s" & @SEC &") -------------------------------"&@CRLF
		$file = FileOpen($folder &"\CRC32 HASHES.txt", 1)
		FileWrite($file, $lines)
		FileClose($file)
		ClipPut($hashes)
		MsgBox(64, "Result", $hashes & @CRLF & "It has been put in your clipboard, you just have to paste it."&@CRLF&"A file named 'CRC32 HASHES.txt' and containing the results has been created in the same folder." )
	Else
		$hash = CRC32($FileName)
		ClipPut($hash)
